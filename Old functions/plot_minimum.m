function [ft,data_out,remap_prop,out_lines] = plot_minimum(param,model,reps,data,l_flag,err,np)
mmn = 200;
x = 1:1600;
...
if length(param)==11
    ...
    fun = str2func("Simulatecont");
else
    fun = str2func("Simulate");
end
...
for rep = 1:reps
    ...
    if ~isempty(data) & exist('stim_in','var')==0
        if isstruct(data)
        ...
        d = [];d2 = [];
    s_fun = @(x)fun(model,param,x,err);
       
    [d.Response,d.Correct,d.Block,d.RT,d.Stimulus, d.First,d.Trial] = s_fun([1 500]);
    d.Exp = ones(1,length(d.Block));
    d = struct2table(flipstruct(d));
    [d2.Response,d2.Correct,d2.Block,d2.RT,d2.Stimulus, d2.First,d2.Trial] = s_fun([4000 500]);
    d2.Exp = 2*ones(1,length(d2.Block));
     d2 = struct2table(flipstruct(d2));
    d = [d;d2];
    end
    remap_prop(rep) = sum(d.Block==3 & d.First==1);
    %end
    %%%  d(d.Exp==1,:)=[];
    if err == 0 & any(d.Correct<0)
