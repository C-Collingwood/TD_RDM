function [NLL,LL,Q,H] = run_hardwicktrialscb(model,init,par_all,data,nlr,free)
...
if sum(bl==3 & f == 1)>200
  ...
  %  NLL = nan;
    ...
    for t = 1:length(bl)
        ...
        elseif  (bl(t)==3) | data.Trial(t)<=50%(t>=e2_start & t<=e2_start+49)%%%
    ...
    NLL = -5*sum(LL(bl==3),'omitnan')-sum(LL(bl~=3),'omitnan');%%-5*sum(mean(LL(bl~=4),'omitnan')+mean(LL(bl==4),'omitnan'));%
end
end
